...
class BusinessSettingsController extends Controller
{
    ...
    public function wholesale_index(Request $request)
    {
        ...
        $sort_search = null;
        $user_type = null;
        $wholesaleCommission = WholesaleCommission::orderBy('max_amount', 'desc')->get();
        //echo "<pre>";print_r($wholesaleCommission);die;
        return view('backend.setup_configurations.wholesale_index', compact('wholesaleCommission'));
    }
    ...
    public function wholesale_update(Request $request)
    {

        if (isset($_POST['min_amount'])) {
            WholesaleCommission::truncate();
            foreach ($_POST['min_amount'] as $key => $value) {
                
                $wholesale_commission = new WholesaleCommission;
                $wholesale_commission->min_amount = (isset($_POST['min_amount'][$key]))?$_POST['min_amount'][$key]:0;;
                $wholesale_commission->max_amount = (isset($_POST['max_amount'][$key]))?$_POST['max_amount'][$key]:0;;
                $wholesale_commission->discount_amount = (isset($_POST['discount_amount'][$key]))?$_POST['discount_amount'][$key]:0;
                $wholesale_commission->discount_percentage = (isset($_POST['discount_percentage'][$key]))?$_POST['discount_percentage'][$key]:0;
                $wholesale_commission->save();
